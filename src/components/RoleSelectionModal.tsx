/**
 * Модальное окно выбора роли и статуса пользователя
 * 
 * Назначение: Позволяет новому пользователю выбрать роль и указать статус после регистрации
 * 
 * Роли:
 * - Перевозчик: у пользователя есть транспорт для перевозки грузов
 * - Логист: работает с обеими сторонами, организует перевозки
 * - Клиент: нужно отправить груз
 * 
 * Статусы:
 * - Для перевозчика: свободен / есть места
 * - Для клиента: готов сейчас / будет готов (с датой и временем)
 * - Для логиста: оба статуса
 * 
 * Особенности:
 * - Двухшаговый процесс: сначала роль, потом статус
 * - Сохранение в backend с fallback на localStorage
 * - Автоматический запрос геолокации пользователя
 * - Адаптивный дизайн с темной темой
 * 
 * Используется в: App.tsx после успешной регистрации нового пользователя
 */
